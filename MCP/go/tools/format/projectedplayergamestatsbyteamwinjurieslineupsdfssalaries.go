...
func ProjectedplayergamestatsbyteamwinjurieslineupsdfssalariesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/PlayerGameProjectionStatsByTeam/%s/%s/%s", cfg.BaseURL, format, season, week, team)
...
func CreateProjectedplayergamestatsbyteamwinjurieslineupsdfssalariesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_PlayerGameProjectionStatsByTeam_season_week_team",
		mcp.WithDescription("Projected Player Game Stats by Team (w/ Injuries, Lineups, DFS Salaries)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProjectedplayergamestatsbyteamwinjurieslineupsdfssalariesHandler(cfg),
	}
}
