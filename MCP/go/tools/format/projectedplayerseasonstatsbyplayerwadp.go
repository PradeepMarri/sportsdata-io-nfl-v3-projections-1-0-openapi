...
func ProjectedplayerseasonstatsbyplayerwadpHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/PlayerSeasonProjectionStatsByPlayerID/%s/%s", cfg.BaseURL, format, season, playerid)
		...
		var result models.PlayerSeasonProjection
...
func CreateProjectedplayerseasonstatsbyplayerwadpTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_PlayerSeasonProjectionStatsByPlayerID_season_playerid",
		mcp.WithDescription("Projected Player Season Stats by Player (w/ ADP)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProjectedplayerseasonstatsbyplayerwadpHandler(cfg),
	}
}
