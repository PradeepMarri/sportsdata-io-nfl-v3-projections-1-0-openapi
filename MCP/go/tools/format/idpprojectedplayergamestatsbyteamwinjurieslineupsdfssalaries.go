...
func IdpprojectedplayergamestatsbyteamwinjurieslineupsdfssalariesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		teamVal, ok := args["team"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: team"), nil
		}
		team, ok := teamVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: team"), nil
		}
		url := fmt.Sprintf("%s/%s/IdpPlayerGameProjectionStatsByTeam/%s/%s/%s", cfg.BaseURL, format, season, week, team)
...
func CreateIdpprojectedplayergamestatsbyteamwinjurieslineupsdfssalariesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_IdpPlayerGameProjectionStatsByTeam_season_week_team",
		mcp.WithDescription("IDP Projected Player Game Stats by Team (w/ Injuries, Lineups, DFS Salaries)"),
		...
		mcp.WithString("team", mcp.Required(), mcp.Description("Abbreviation of the team. Example: <code>WAS</code>.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    IdpprojectedplayergamestatsbyteamwinjurieslineupsdfssalariesHandler(cfg),
	}
}
