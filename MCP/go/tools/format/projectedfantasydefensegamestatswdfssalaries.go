...
func ProjectedfantasydefensegamestatswdfssalariesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/FantasyDefenseProjectionsByGame/%s/%s", cfg.BaseURL, format, season, week)
		...
		var result []FantasyDefenseGameProjection
...
func CreateProjectedfantasydefensegamestatswdfssalariesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_FantasyDefenseProjectionsByGame_season_week",
		mcp.WithDescription("Projected Fantasy Defense Game Stats (w/ DFS Salaries)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProjectedfantasydefensegamestatswdfssalariesHandler(cfg),
	}
}
