...
func ProjectedfantasydefenseseasonstatswadpHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/FantasyDefenseProjectionsBySeason/%s", cfg.BaseURL, format, season)
		...
		var result []FantasyDefenseSeasonProjection
...
func CreateProjectedfantasydefenseseasonstatswadpTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_FantasyDefenseProjectionsBySeason_season",
		mcp.WithDescription("Projected Fantasy Defense Season Stats (w/ ADP)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProjectedfantasydefenseseasonstatswadpHandler(cfg),
	}
}
