...
func DfsslatesbyweekHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/DfsSlatesByWeek/%s/%s", cfg.BaseURL, format, season, week)
...
func CreateDfsslatesbyweekTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_DfsSlatesByWeek_season_week",
		mcp.WithDescription("DFS Slates by Week"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DfsslatesbyweekHandler(cfg),
	}
}
