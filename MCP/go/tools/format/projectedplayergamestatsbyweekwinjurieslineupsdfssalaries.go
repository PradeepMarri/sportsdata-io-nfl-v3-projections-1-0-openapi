...
func ProjectedplayergamestatsbyweekwinjurieslineupsdfssalariesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/PlayerGameProjectionStatsByWeek/%s/%s", cfg.BaseURL, format, season, week)
...
func CreateProjectedplayergamestatsbyweekwinjurieslineupsdfssalariesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_PlayerGameProjectionStatsByWeek_season_week",
		mcp.WithDescription("Projected Player Game Stats by Week (w/ Injuries, Lineups, DFS Salaries)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ProjectedplayergamestatsbyweekwinjurieslineupsdfssalariesHandler(cfg),
	}
}
