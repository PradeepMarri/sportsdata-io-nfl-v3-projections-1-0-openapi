...
func IdpprojectedplayergamestatsbyplayerwinjurieslineupsdfssalariesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/%s/IdpPlayerGameProjectionStatsByPlayerID/%s/%s/%s", cfg.BaseURL, format, season, week, playerid)
...
func CreateIdpprojectedplayergamestatsbyplayerwinjurieslineupsdfssalariesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_IdpPlayerGameProjectionStatsByPlayerID_season_week_playerid",
		mcp.WithDescription("IDP Projected Player Game Stats by Player (w/ Injuries, Lineups, DFS Salaries)"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    IdpprojectedplayergamestatsbyplayerwinjurieslineupsdfssalariesHandler(cfg),
	}
}
